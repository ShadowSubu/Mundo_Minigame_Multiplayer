...
{
    [SerializeField] private Animations chatAnimations;
    ...
    private void OnChatInput(char character)
    {
        //StartChatInactivityTimer();

        ChatSystem.Instance.RequestToSendChatInputRpc(character, NetworkManager.Singleton.LocalClientId);
    }
    ...
    private void ChatSystem_OnChatInputRecieved(object sender, ChatSystem.ChatMessageEventArgs e)
    {
        ...
        //DisplayChatMessage(e.character, e.senderCliendId);
        chatAnimations.EnqueueCharacter(e.character, e.senderCliendId);
    }
